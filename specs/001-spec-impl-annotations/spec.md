...
- **FR-003**: 注釈はステータスモデル（例: 実装済み / 進行中 / 未実装 / 廃止）を持てること。[NEEDS CLARIFICATION: 単純なブール値（実装済み/未実装）でよいか、多段階ステータスを採用するかを選択]
  - **Acceptance**: ステータスを変更すると、一覧のフィルタとレポートに反映される。
...
- **Q2 (FR-003)**: ステータスは単純な実装済み/未実装のブールで良いですか、それとも `実装済み/進行中/未実装/廃止` のような多段階ステータスを用いますか？

(最大3つまでの [NEEDS CLARIFICATION] を許容します。Q2 をご回答ください)
